...
let facts_to_json (facts: facts_db) : Yojson.Basic.t =
  `Assoc [
...
let json_to_facts (j: Yojson.Basic.t) : facts_db =
  let open Yojson.Basic.Util in
...
let rec handle_query_json (json_str: string) (env: type_environment) : string =
  try
    ...
    let formula = 
      try
        Parser.main Lexer.read (Lexing.from_string request.formula_string)
      with e ->
    ...
    (* Setup databases *)
    let domain = { entities = extract_entities_from_facts request.facts } in
    let funcs = { func_values = [] } in (* Empty for now *)
...
and extract_entities_from_facts (facts: facts_db) : string list =
...
let run_stdio_mode (env: type_environment) : unit =
  try
    ...
    if String.length input = 0 then
      ...
      let output = handle_query_json input env in
...
let run_file_mode (filename: string) (env: type_environment) : unit =
  try
