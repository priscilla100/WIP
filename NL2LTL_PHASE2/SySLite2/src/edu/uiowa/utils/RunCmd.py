...
def run_sygus(_adt_def, start_time, trace_file,  max_loop = 2, bv_encoding = True):
    ...
    logging.debug('Running SyGuS(CVC4)...')
...
def run_cmd(cmd, max_loop, start_time, bv_type):
    ...
    try:
        ...
        for line in process.stdout:
            logging.debug('Synthesized Formula Definition %s'%(line))
...
##                f = parser.parse_ltl_bv(read_data)
                result.append(f)
            else:#adt_type
                f = parser.parse_adt(read_data)    
                result.append(f)
                
            logging.info('Found Formula %s'%(f))
