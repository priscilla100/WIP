...
AP_Lit, POS_TRACE, NEG_TRACE, UNARY_OP, BINARY_OP, OP, SIZE, TARGET_FML, = range(1, 9)
...
def read_traces_3(fp, isLit = False):
    ...
    while line:
        ...
        if data == '---':                
            trace_type = next_type1(trace_type)
            line = fp.readline()            
            continue
...
#            if not target is None: 
#                parser = pLTLParser()
            fml = target#parser.parse(target)
