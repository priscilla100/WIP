...
#        bv_def = bv_def.replace(';%{3}', define_vars(self.nvars, self.AP))        
    ...
    def bv_sygus_ltl_def(self, bv_def):
    
        #Variables 
#        print('0000000000000000000000000000000*********', self.vars)
        self.vars = self.vars + 1
        
        bv_def = bv_def.replace(';%{1}', define_ltl_streams(self.vars))    
        bv_def = bv_def.replace(';%{2}', define_last_index(self.vars))

        #Add reverse
        bv_def = bv_def.replace(';%{3}', define_rev(self.vars))

        #Variables in SyGus Grammar
        bv_def = bv_def.replace(';%{4}', define_vars_stream(self.AP))
        bv_def = bv_def.replace(';%{4a}', define_vars_stream_p(self.AP))
...
#        bv_def = bv_def.replace(';%{3}', define_vars(self.nvars, self.AP))        
        #Trace Values
        bv_def = bv_def.replace(';%{5}', enum_ltl_traces(self.pos_traces, self.nvars, self.vars, True, self.vars))
             
        #Length of Positive Trace
        if len(self.neg_traces) > 0:#
            bv_def = bv_def.replace(';%{6}', enum_ltl_traces(self.neg_traces, self.nvars, self.vars, False, self.vars))
...
# Define LTL Attributes 
def define_ltl_streams(nvars):
    
    _steam_def = define_streams(nvars)
    _bvn = '(define-sort BVn () (_ BitVec %d))\n'%(nvars)

    _stream_def = ''.join(_bvn + _steam_def)       
    
    return _stream_def

def define_last_index(nvars):

    _lastindex = '(define-fun LastIndex () Stream (_ bv%d %d))\n'%(nvars-1,nvars)

    return _lastindex

def define_rev(nvars):
    concat_ext = _concat(0, nvars-1)
    _rev = '(define-fun rev ((A Stream)) Stream \n%s )\n'%(concat_ext)
    return _rev 

def _concat(sindex, nvars):    
    if sindex == nvars:
        return _extract(sindex)
    else:
        _rev= '(concat %s %s)'%(_extract(sindex), _concat(sindex+1, nvars)) 
        return _rev
    
def _extract(value):
    _extracted_stream = '((_ extract %d %d) A)\n'%(value,value)
    return _extracted_stream

    
def enum_ltl_traces(_traces, nvars, vars, _type, max_vars):
    _trace_data = []
...
#        print('*******(max_vars)', max_vars)
        ...
        trace_value = trace_bv_loop_data(trace_value, nvars, max_vars,tr.lasso)
#(= ((_ extract 1 0) (phi #b000 #b000)) ((_ extract 1 0) S_TRUE))                
        if _type:
            trace_len = tr.traceLength - tr.lasso  
#            print('>>>>>>>>>>Prefix prefix length, trace length, lasso_index', trace_len, tr.traceLength, tr.lasso)
#            if trace_len < max_vars:
            
            _value = '    (= ((_ extract 0 0) (phi (_ bv%d %d) (_ bv%d %d) %s)) #b1)'%(trace_len, vars, tr.lasso, vars, trace_value)

#                _value = '    (= ((_ extract 0 0) (phi (_ bv%d %d) (_ bv0 %d) %s)) #b1)'%(trace_len - 1, trace_len, trace_len, trace_value, trace_len - 1)
#            else:
...
#            _value = '    (= (phi %s) S_TRUE)'%(trace_value)
            ...
            trace_len = tr.traceLength  - tr.lasso 
#            if trace_len < max_vars:            
            _value = '    (= ((_ extract 0 0) (phi (_ bv%d %d) (_ bv%d %d) %s)) #b0)'%(trace_len, vars, tr.lasso, vars, trace_value)
#            else:
...
def trace_bv_loop_data(traces, nvars, max_length, lasso):
    #while v_index < nvars:
    trace_value  = []
    v_index = 0
#    (= (phi
...
#            var_value.append(vector[v_index])
        ...
        loop_bit = var_value[lasso]
        var_value = var_value[::-1]

#        var_value.append(loop_bit)
        
        pad = '0'* (max_length - (values_length + 1 ))                    
        v = ''.join(var_value)
#        print('value',v)
#        print('lasso:', lasso)
        if values_length < max_length:
